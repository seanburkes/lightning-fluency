...
class LanguageServiceImpl(
  ...
  override fun createLanguage(dto: CreateLanguageDto): LanguageDto {
    validateLanguageNameRequired(dto.name)

    if (isDuplicateLanguageName(dto.name, excludeId = null)) {
      throw DuplicateLanguageException("Language with name '${dto.name}' already exists")
    }

    if (!validateParserType(dto.parser_type)) {
  ...
  override fun updateLanguage(id: Long, dto: UpdateLanguageDto): LanguageDto? {
    ...
    dto.name?.let { name ->
      validateLanguageNameRequired(name)

      if (isDuplicateLanguageName(name, excludeId = id)) {
        throw DuplicateLanguageException("Language with name '$name' already exists")
      }
    }

    dto.parser_type?.let {
      if (!validateParserType(it)) {
  ...
  private fun validateLanguageNameRequired(name: String) {
    if (name.isBlank()) {
  ...
  private fun isDuplicateLanguageName(name: String, excludeId: Long?): Boolean {
    val existing = languageRepository.findByName(name)
